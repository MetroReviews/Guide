  ...
  const recursiveFilter = (incomingNav: DocsNav, parentIndex?: number) => {
    if (incomingNav) {
      incomingNav.forEach((item, index) => {
        const { label, href, items, type } = item
...
export const NextInDocs: React.FC<{
  ...
  useEffect(() => {
    ...
    if (nextIndex && navLinks[nextIndex]) {
      ...
      const { index, parentIndex } = nextNavLink
  ...
  if (nextNavItem) {
    const { href, label } = nextNavItem

    return (
      <Hyperlink
        href={href}
        underline={false}
        className={[className, classes.nextInDocs, isDark && classes.isDark]
          .filter(Boolean)
          .join(' ')}
      >
        <div>Next:</div>
