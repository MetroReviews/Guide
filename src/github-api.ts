...
export const getGitHubMasterTree = async (repoName: string): Promise<GitHubTree> => {
  const res = await fetch(
    `https://api.github.com/repos/MetroReviews/${repoName}/git/trees/master?recursive=1`,
    {
      method: 'GET',
...
export const getGitHubVersionNumber = async (repo: string): Promise<string> => {
  const res = await fetch(`https://api.github.com/repos/MetroReviews/${repo}/releases/latest`, {
    method: 'GET',
    headers: defaultHeaders,
  })
